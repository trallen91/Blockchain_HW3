...
class SimplePKIBA:
    ...
    def calculate_votes_for(self, round):
        ...
        all_valid_props = self.get_proposals_with_threshold(round)
        # placeholder for (3.1)
        ...
        new_props = list(set(all_valid_props).difference(self.s_i))
    ...
    def get_proposals_with_threshold(self, round):
        ...
        ret_val = []
        for prop, sigs in self.votes.items():
            sender_signed = self.sender in sigs
            threshold_reached = len(set(sigs)) >= round
            if threshold_reached & sender_signed:
                ret_val.append(prop)
    ...
    def get_output(self):
        ...
        if (len(self.s_i) > 1) | (len(self.s_i) == 0):
            return 0
        else:
            return self.s_i[0]
