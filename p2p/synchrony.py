...
def get_curr_round():
    ...
    if start_time == None: return start_time
    # Do not round intermediate arithmetic
    # placeholder for (2.2)
    return int((time.time() - start_time)/round_length)
...
def should_send():
    ...
    round = get_curr_round()
    round_time = round_length * round

    if (round_time+synchrony_assumption<=time_diff<=round_time+synchrony_assumption*2):return True
    # placeholder for (2.3)
    return False
